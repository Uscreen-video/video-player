...
const videoTextTtracksManager = (video: HTMLVideoElement, hls: Hls) => {
  ...
  const langToSrcMapping = trackElements.reduce<Record<string, string>>((acc, t) => {
    acc[t.srclang] = t.src
    return acc
  }, {})

  const isTrackNative = (track: TextTrack) => !!trackElements.find(t => t.label === track.label && t.srclang === track.language)
...
export const subtitlesController = (
  ...
  const onTextTrackAdded = (data: TrackEvent) => {
    subtitlesDebug('TRACK ADDED', data.track.label, data.track.language, data.track.kind)
  ...
  return {
    enableTextTrack: (lang: string) => {
      ...
      tracksManager.showTracks(activeTextTrack)
      const activeTrack = tracksManager.getTracks().find(t => (t.language || t.label) === activeTextTrack)
      if (activeTrack && tracksManager.isTrackNative(activeTrack) && activeTrack.activeCues) {
